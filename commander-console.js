...
export function initCommanderConsole({getDashboard,openOfficer,openMetric,renderGroup}){
  ...
  function revealConversation(){requestAnimationFrame(()=>conversation.scrollIntoView({behavior:'smooth',block:'center'}))}
  function addMessage(role,text,{reveal=false}={}){const d=document.createElement('div');d.className='msg '+(role==='commander'?'commander':'ship');const b=document.createElement('b');b.textContent=role==='commander'?'COMANDANTE RUDY':'STARSHIP';d.appendChild(b);d.append(document.createTextNode(text));conversation.appendChild(d);conversation.scrollTop=conversation.scrollHeight;if(reveal)revealConversation()}
  function resetPending(){pending=null;confirmBox.hidden=true;confirmText.textContent=''}
  function stage(text){const raw=String(text||'').trim();if(!raw)return;addMessage('commander',raw);input.value='';const cmd=interpretCommand(raw);if(cmd.type==='UNKNOWN'){addMessage('ship','Questo ordine non è ancora collegato. Puoi riformularlo o proseguire la conversazione.',{reveal:true});resetPending();return}if(cmd.type==='EMPTY'){resetPending();return}pending=cmd;confirmText.textContent=cmd.label;confirmBox.hidden=false}
  ...
  cancel.addEventListener('click',()=>{resetPending();input.value='';addMessage('ship','Ordine cancellato.',{reveal:true});input.focus()});
  confirm.addEventListener('click',execute);
  cont.addEventListener('click',()=>{resetPending();input.focus()});
  clear.addEventListener('click',()=>{conversation.innerHTML='<div class="msg ship"><b>STARSHIP</b>Conversazione pulita. Console pronta.</div>';resetPending();input.value='';input.focus();revealConversation()});
  const SR=window.SpeechRecognition||window.webkitSpeechRecognition;
  if(!SR){mic.disabled=true;mic.textContent='MICROFONO NON DISPONIBILE';}
  else{recognition=new SR();recognition.lang='it-IT';recognition.interimResults=false;recognition.maxAlternatives=1;recognition.onstart=()=>{listening=true;mic.classList.add('active');mic.textContent='ASCOLTO…'};recognition.onend=()=>{listening=false;mic.classList.remove('active');mic.textContent='🎙 MICROFONO'};recognition.onerror=e=>addMessage('ship','Microfono: '+(e.error||'errore'),{reveal:true});recognition.onresult=e=>{const t=e.results?.[0]?.[0]?.transcript||'';input.value=t;stage(t)};mic.addEventListener('click',()=>{if(listening)recognition.stop();else recognition.start()})}
}
